...
def load_data(data_path="C:/Nguyen Tri/Code/Statisanalyss/all_training_data.csv"):
...
def apply_pacf_and_filter(df):
    ...
    Isolate true tire decay from random traffic noise.
    """
    print("\n--- 1. Time Series Analysis (PACF) ---")
    
    df = df.sort_values(by=['Driver', 'Stint', 'LapNumber']).copy()
    
    # Store stationary pace decay
    df['Stationary_PaceDecay'] = np.nan
    
    pacf_plotted = False
    
    # Process stint by stint
    for (driver, stint), group in df.groupby(['Driver', 'Stint']):
        if len(group) > 5:
            lap_times = group['LapTime_Seconds'].values
            # Run PACF to find lag p
            try:
                pacf_vals = pacf(lap_times, nlags=min(10, len(lap_times)//2 - 1))
                # Applying first-difference AR(1) filter
                # \Delta X_t = X_t - \phi_1 X_{t-1}
                decay = group['LapTime_Seconds'].diff()
                df.loc[group.index, 'Stationary_PaceDecay'] = decay
                
                if not pacf_plotted:
                    fig = plot_pacf(lap_times, lags=min(15, len(lap_times)//2 - 1), title=f"PACF of Lap Times (Driver {driver}, Stint {stint})")
                    plt.savefig("PACF_Sample_Stint.png")
                    plt.close(fig)
                    pacf_plotted = True
...
def analyze_tyrelife_variance(df):
    ...
    Quantifying Strategic Uncertainty via E[X] and Var(X) of Tyrelife.
    """
    print("\n--- 3. Random Variables & Target Modeling ---")
    ...
    sns.boxplot(data=pit_stops, x='Team', y='TyreLife', order=order, hue='Team', legend=False, palette='viridis')
    plt.title('Tyrelife Distribution by Team (Ordered by Variance)')
    plt.xticks(rotation=45)
    plt.tight_layout()
    plt.savefig('Tyrelife_Variance_by_Team.png')
    plt.close()
...
def build_hazard_model(df):
    ...
    Model pit stop sequence as a survival system (Hazard Rate h(t)).
    """
    print("\n--- 2. Stochastic Processes & Hazard Modeling ---")
    ...
    # We take the final row of each stint for static survival analysis
    ...
    try:
        ...
        # Predict Pit_Probability (Normalized Partial Hazard)
        predict_df = df[features].copy()
        predict_df['Stationary_PaceDecay'] = predict_df['Stationary_PaceDecay'].fillna(0)
        predict_df['traffic_pressure'] = predict_df['traffic_pressure'].fillna(predict_df['traffic_pressure'].median())
        partial_hazard = cph.predict_partial_hazard(predict_df)
        df['Pit_Probability'] = (partial_hazard - partial_hazard.min()) / (partial_hazard.max() - partial_hazard.min() + 1e-9)
        return df
    except Exception as e:
...
def evaluate_errors(df):
    ...
    Bias-Variance MSE decomposition and Welch's t-tests.
    """
    print("\n--- 4. Parameter Estimation & Error Evaluation ---")
    ...
    sns.boxplot(data=plot_data, x='Traffic Pressure', y='Stationary_PaceDecay', hue='Traffic Pressure', legend=False, palette='coolwarm')
    plt.title("Pace Decay: High vs Low Traffic Pressure")
    plt.tight_layout()
    plt.savefig('Pace_Decay_vs_Traffic.png')
    plt.close()
