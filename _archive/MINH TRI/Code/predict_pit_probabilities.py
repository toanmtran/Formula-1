...
OUTPUT_PATH = "C:/Nguyen Tri/Code/Statisanalyss/all_training_data.csv" # Overwriting original file
MODEL_PATH = "C:/Nguyen Tri/Code/Statisanalyss/best_f1_model.pt"
...
def main():
    ...
    # =========================================================
    # PREPROCESSING (Mirroring train.py logic exactly)
    # =========================================================
    drop_cols = [
    ...
    encoded_cat_cols = [col for col in df.columns if col.startswith("Compound_") or col.startswith("Team_")]
    ...
    # Add an index column to guarantee exact row mapping back to original_df
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            # Squeeze dim 1 (the 1 target dimension) and apply sigmoid
            probs = torch.sigmoid(outputs).squeeze(-1).cpu().numpy()
            
            # Handle possible scalars if batch size is 1
            if probs.ndim == 0:
    ...
    # Set default probability to 0.0 (for first 19 laps of a stint and short stints)
    original_df['PitStopProbability'] = 0.0
    
    # Safely assign the calculated probabilities directly using the row indices we saved!
    original_df.loc[predicted_indices, 'PitStopProbability'] = all_probs

    print(f"Saving updated data to {OUTPUT_PATH}...")
    original_df.to_csv(OUTPUT_PATH, index=False)
    print("Success! The 'PitStopProbability' column has been updated in all_training_data.csv")
