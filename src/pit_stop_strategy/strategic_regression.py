...
from src.pit_stop_strategy.paths import ERGAST_DIR as _ERGAST_DIR, AGGREGATED_CSV

ERGAST_DIR = str(_ERGAST_DIR)
FASTF1_DIR = str(AGGREGATED_CSV)
...
def analyze_pit_stop_correlation():
    ...
    # Drop > 40s as repair / penalty laps rather than service stops
    ...
    # Restrict to teams with > 100 career points so pre-1960 noise doesn't dominate
    ...
    print(team_stats.sort_values('duration_sec').head()
          [['name', 'duration_sec', 'points']].to_string(index=False))
...
def analyze_tactical_agility():
    ...
    for (race, driver), group in df.groupby(['Race', 'Driver']):
        ...
        # No-reaction penalty: 5 laps stands in for "did not pit under VSC"
    ...
    if not SKLEARN_AVAILABLE:
        print("scikit-learn not installed. Skipping RandomForest model training.")
        return
